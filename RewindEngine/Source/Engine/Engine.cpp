...
void Engine::AllocateSystems()
{
	...
	ISystem * audioManager = new AudioManager();
	if (audioManager)
		m_systems[audioManager->GetName()] = audioManager;

	ISystem* input = new Input::InputHandler();
	if (input)
		m_systems[input->GetName()] = input;
}
...
void Engine::Run()
{
	...
	AudioManager * audioManager = dynamic_cast<AudioManager*>(GetSystem("AudioManager"));
	audioManager->LoadSound("Resources\\breakout.mp3","breakout");
	audioManager->Play("breakout");
	...
	while (IsRunning())
	{
		...
		while (PeekMessage(&msg, 0, 0, 0, PM_REMOVE))
		{
			...
			{
				ISound* music = audioManager->GetSoundEngine()->play2D("Resources\\chinese.mp3", true, false, true, ESM_AUTO_DETECT, true);
				ISoundEffectControl* fx = 0;
				if (music)
					fx = music->getSoundEffectControl();
				//audioManager->PauseSound("breakout", false);
...
LRESULT WINAPI Engine::MessageHandler(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_SIZE:
	{
		...
		GraphicSystem * graphics = dynamic_cast<GraphicSystem*>(GetSystem("Graphics"));
		if(graphics)
